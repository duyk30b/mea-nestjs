import { EGender } from '../../../database/src/common/variable'
import { timeToText } from '../formatters/time.formatter'
import { convertViToEn } from './string.helper'

export const shuffleArray = <T>(origin: T[]): T[] => [...origin].sort(() => 0.5 - Math.random())

export const randomItemsInArray = <T>(items: T[]): T => items[Math.floor(Math.random() * items.length)]
...
export const randomUsername = (fullName?: string, birthday?: Date): string => {
	...
	const number = timeToText(birthday, 'DDMMYY')
	return text + number
}
