...
export class AccessLogInterceptor implements NestInterceptor {
    ...
    intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
        ...
        if (context.getType() === 'http') {
            ...
            msgRequest
                = `[HTTP] | ${method} | ${originalUrl} | ${className} | ${funcName}()`
            ...
            if (res.constructor.name === 'NatsContext') {
                ...
                msgRequest
                    = `[NATS] | ${response.getSubject()} | ${className} | ${funcName}()` + ` | ${JSON.stringify(req)} `
            }
            if (res.constructor.name === 'KafkaContext') {
                ...
                msgRequest = `[KAFKA] | ${JSON.stringify(topicInfo)} | ${className} | ${funcName}() | ${JSON.stringify(
                    req
                )} `
            }
        }
