...
import { InvoiceItemType, InvoiceStatus, ReceiptStatus } from '_libs/database/common/variable'
import { Customer, Invoice, InvoiceExpense, InvoiceItem, InvoiceSurcharge, Receipt } from '_libs/database/entities'
...
export class StatisticRepository {
    ...
    async sumMoneyInvoice(options: { oid: number; fromTime: Date; toTime: Date; timeType: 'date' | 'month' }) {
        ...
        if (timeType === 'date') {
            query = query.groupBy('shipYear').addGroupBy('shipMonth').addGroupBy('shipDate')
    ...
    async sumInvoiceExpense(options: { oid: number; fromTime: Date; toTime: Date; timeType: 'date' | 'month' }) {
        ...
        if (timeType === 'date') {
            query = query.addGroupBy('shipYear').addGroupBy('shipMonth').addGroupBy('shipDate')
        } else if (timeType === 'month') {
