...
export class version221699519328548 implements MigrationInterface {
	...
	public async up(queryRunner: QueryRunner): Promise<void> {
		...
		if ('Invoice') {
			...
			await queryRunner.query(`
				UPDATE customer JOIN invoice
					...
					AND invoice.create_time = (SELECT MAX(create_time) FROM invoice iv WHERE iv.customer_id = customer.id AND iv.status = 3)
		...
		if ('DistributorDebt ===> Distributor Payment') {
			...
			`)
			await queryRunner.query(`
				CREATE INDEX \`IDX_DISTRIBUTOR_PAYMENT__DISTRIBUTOR_ID\` ON \`distributor_debt\` (\`oid\`, \`distributor_id\`)
			`)
			await queryRunner.query(`
				CREATE INDEX \`IDX_DISTRIBUTOR_PAYMENT__RECEIPT_ID\` ON \`distributor_debt\` (\`oid\`, \`receipt_id\`)
			`)
			await queryRunner.query(`
		...
		if ('Customer Debt ===> Customer Payment') {
			...
			`)
			await queryRunner.query(`
				CREATE INDEX \`IDX_CUSTOMER_PAYMENT__CUSTOMER_ID\` ON \`customer_payment\` (\`oid\`, \`customer_id\`)
			`)
			await queryRunner.query(`
				CREATE INDEX \`IDX_CUSTOMER_PAYMENT__INVOICE_ID\` ON \`customer_payment\` (\`oid\`, \`invoice_id\`)
			`)
	...
	public async down(queryRunner: QueryRunner): Promise<void> {
		...
		if ('DistributorDebt ===> Distributor Payment') {
			...
			`)
			await queryRunner.query(`
				CREATE INDEX \`IDX_58d7593e401ba54185a6f1617b\` ON \`customer_debt\` (\`oid\`, \`customer_id\`)
			`)
		}
		if ('Customer Debt ===> Customer Payment') {
			await queryRunner.query(`
			...
			`)
			await queryRunner.query(`
				CREATE INDEX \`IDX_30800e79e2cab0345762b42f11\` ON \`distributor_debt\` (\`oid\`, \`distributor_id\`)
			`)
		}
		if ('InvoiceItem') {
			await queryRunner.query(`
