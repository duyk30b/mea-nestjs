...
export class ApiProductBatchController {
	...
	@Get('detail/:id')
	async detail(@External() { oid }: TExternal, @Param() { id }: IdParam, @Query() query: ProductBatchGetOneQuery) {
	...
	@Patch('update/:id')
	async update(@External() { oid }: TExternal, @Param() { id }: IdParam, @Body() body: ProductBatchUpdateBody) {
