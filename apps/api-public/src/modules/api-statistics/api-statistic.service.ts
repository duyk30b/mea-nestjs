...
export class ApiStatisticService {
    ...
    async topProductBestSelling(oid: number, query: StatisticTopBestSellingQuery) {
    ...
    async topProcedureBestSelling(oid: number, query: StatisticTopBestSellingQuery) {
        const { fromTime, toTime, orderBy, limit } = query

        return await this.statisticRepository.topProcedureBestSelling({
            oid,
            fromTime: fromTime.getTime(),
            toTime: toTime.getTime(),
            orderBy,
    ...
    async topCustomerBestInvoice(oid: number, query: StatisticTopCustomerBestInvoiceQuery) {
        const { fromTime, toTime, orderBy, limit } = query
    ...
    async sumMoneyReceipt(oid: number, query: StatisticTimeQuery) {
        const { fromTime, toTime, timeType } = query
    ...
    async sumInvoiceSurchargeAndExpenses(oid: number, query: StatisticTimeQuery) {
        const { fromTime, toTime, timeType } = query
        ...
        // tạo ra 1 dataMap có đầy đủ các giá trị = 0
        const surchargeMap: Record<string, { name: string; data: Record<string, { sumMoney: number }> }> = {}
        ...
        return { surcharge: surchargeMap, expense: expenseMap }
    }
    ...
    async revenueMonth(oid: number, year: number, month: number) {
        ...
        const invoices = await this.invoiceRepository.findMany({
            oid,
            time: ['BETWEEN', new Date(startMonth), new Date(endMonth)],
