...
export class SocketGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
    ...
    async handleConnection(socket: Socket, ...args: any[]) {
        ...
        try {
            ...
            this.logger.log(`IP ${ip} UserId ${uid} with socketId ${socket.id} connected, join room ${oid}`)
        } catch (error) {
    ...
    async arrivalCreate(socket: Socket, data: any): Promise<any> {
        this.io.in(socket.data.user.oid.toString()).emit(SOCKET_EVENT.ADMISSION_NEW, `${data}, ${Date.now()}`)
        return Date.now()
    }
}
