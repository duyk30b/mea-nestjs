...
export class AuthService {
    ...
    async register(registerDto: RegisterBody): Promise<User> {
        ...
        const employee = await this.dataSource.transaction('READ UNCOMMITTED', async (manager) => {
            ...
            const employeeSnap = manager.create(User, {
                oid: organization.id,
                username,
    ...
    async login(loginDto: LoginBody): Promise<User> {
        const [employee] = await this.dataSource.getRepository(User).find({
    ...
    async forgotPassword(body: ForgotPasswordBody): Promise<any> {
        ...
        const employee = await this.dataSource.manager.findOne(User, {
            where: {
