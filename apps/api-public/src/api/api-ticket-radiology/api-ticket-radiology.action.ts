...
import { TicketRadiologyPostQuery, TicketRadiologyUpdateResultBody } from './request'
...
export class ApiTicketRadiologyAction {
  ...
  async updateResult(options: {
    ...
    this.socketEmitService.ticketRadiologyListChange(oid, {
      ticketId,
  ...
  async cancelResult(options: { oid: number; ticketRadiologyId: number }) {
    const { oid, ticketRadiologyId } = options
