...
> extends BaseMongoCondition<_SCHEMA> {
  ...
  async pagination<S extends _SORT, R extends _RELATION>(options: {
    ...
    const filter = rawCondition ? condition || {} : this.getFilterOptions(condition)
    ...
    const data: _TYPE[] = docs.map((i) => i.toObject() as any)
  ...
  async findOne<S extends _SORT, R extends _RELATION>(options: {
    ...
    return doc ? (doc.toObject() as any) : null
  }
  ...
  async insertOneBasic<T extends Partial<_INSERT>>(
  ...
  async insertOne<T extends _INSERT>(data: NoExtra<_INSERT, T>): Promise<_TYPE> {
    const model = new this.model(data)
  ...
  async insertManyBasic<T extends Partial<_INSERT>>(
    data: NoExtra<Partial<_INSERT>, T>[]
  ): Promise<_TYPE[]> {
  ...
  async insertMany<T extends _INSERT>(data: NoExtra<_INSERT, T>[]): Promise<_TYPE[]> {
    const hydratedDocument = await this.model.insertMany(data)
    const result = hydratedDocument.map((i: any) => i.toObject())
    return result
  }
  ...
  async updateMany<T extends Partial<_UPDATE>>(
    ...
    const filter = this.getFilterOptions(condition)
    const result = await this.model.updateMany(filter, data as unknown as UpdateQuery<_SCHEMA>, {
      upsert: false,
    })
