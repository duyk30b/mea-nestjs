...
> {
    ...
    async pagination(options: { skip: number; limit: number; hasCount?: boolean; condition?: FilterQuery<_SCHEMA> }) {
    ...
    async updateOne<T extends Partial<_TYPE>>(
        ...
        const filter = this.getFilterOptions(condition)
        const hydratedDocument = await this.model.findOneAndUpdate(filter, data as unknown as UpdateQuery<_SCHEMA>, {
            new: true,
        })
