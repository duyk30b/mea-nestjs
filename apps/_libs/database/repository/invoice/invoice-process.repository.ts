...
import { InvoiceItemType, InvoiceStatus, PaymentType } from '../../common/variable'
import {
  Customer,
  CustomerPayment,
  Invoice,
...
export class InvoiceProcessRepository {
  ...
  async startShipAndPayment(params: {
    ...
    const transaction = await this.dataSource.transaction('READ UNCOMMITTED', async (manager) => {
      ...
      const invoiceItemsProduct = invoice.invoiceItems.filter(
        (i) => i.type === InvoiceItemType.ProductBatch
      )
      const invoiceItemIds = invoiceItemsProduct.map((i) => i.id)
      const productBatchIds = uniqueArray(
        invoiceItemsProduct.map((i: InvoiceItem) => i.referenceId)
      )
      ...
      if (invoiceItemsProduct.length) {
        ...
        const productMovementsSnap = invoiceItemsProduct.map((invoiceItem) => {
          ...
          if (!productBatch) {
            throw new Error(
              `Process Invoice ${invoiceId} failed: ` +
                `ProductBatchID ${invoiceItem.referenceId} invalid`
            )
          }
    ...
    const transaction = await this.dataSource.transaction('REPEATABLE READ', async (manager) => {
      ...
      if (invoiceItemsProduct.length) {
        ...
        const updateBatch: [any[], number] = await manager.query(`
                    ...
                    WHERE   "productBatch"."id" = "sii"."referenceId"
                                AND "productBatch"."id" IN (${productBatchIds.toString()})
                                AND "productBatch"."oid" = ${oid}
                `)
        ...
        if (updateBatch[1] !== productBatchIds.length) {
          throw new Error(
            `Refund Ship Invoice ${invoiceId} failed: Some batch can't update quantity`
          )
        }
        ...
        const productMovementsSnap = invoiceItemsProduct.map((invoiceItem) => {
          ...
          if (!productBatch) {
            throw new Error(
              `Refund Invoice ${invoiceId} failed: ` +
                `ProductBatchID ${invoiceItem.referenceId} invalid`
            )
          }
          // cần cập nhật số lượng vì 1 lô có thể bán 2 số lượng với 2 giá khác nhau
