...
export class ReceiptProcessRepository {
  ...
  async createDraft(params: { oid: number; receiptInsertDto: ReceiptInsertDto }) {
    ...
    return await this.dataSource.transaction('READ UNCOMMITTED', async (manager) => {
      const receiptSnap = manager.create(Receipt, receiptInsertDto)
      receiptSnap.oid = oid
  ...
  async updateDraft(params: {
    ...
    return await this.dataSource.transaction('READ UNCOMMITTED', async (manager) => {
      const { receiptItems, ...receiptSnap } = manager.create(Receipt, receiptUpdateDto)
      receiptSnap.paid = 0
      receiptSnap.debt = 0
    ...
    const transaction = await this.dataSource.transaction('READ UNCOMMITTED', async (manager) => {
      ...
      `)
        ...
        const productMovementsEntity = receipt.receiptItems.map((receiptItem) => {
          ...
          if (!productBatch) {
            throw new Error(
              `Process Receipt ${receiptId} failed: ` +
                `ProductBatchID ${receiptItem.productBatchId} invalid`
            )
          }
