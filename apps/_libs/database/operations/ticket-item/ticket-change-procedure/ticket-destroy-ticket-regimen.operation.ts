...
export class TicketDestroyTicketRegimenOperation {
  ...
  async destroyTicketRegimen(params: { oid: number; ticketId: number; ticketRegimenId: number }) {
    ...
    const transaction = await this.dataSource.transaction('READ UNCOMMITTED', async (manager) => {
      ...
      const ticketRegimenDestroyed = await this.ticketRegimenManager.deleteOneAndReturnEntity(
        manager,
        {
          oid,
          ticketId,
      ...
      const ticketProcedureDestroyedList = await this.ticketProcedureManager.deleteAndReturnEntity(
        manager,
        { oid, ticketRegimenId }
      )
      ...
      return { ticketModified, ticketRegimenDestroyed, ticketUserDestroyedList }
    })
