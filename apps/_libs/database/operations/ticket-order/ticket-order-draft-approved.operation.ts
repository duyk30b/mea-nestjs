...
@Injectable()
export class TicketOrderDraftApprovedOperation {
  constructor(
  ...
  async update<T extends TicketOrderDraftApprovedUpdateType>(params: {
    oid: number
    ticketId: number
    ticketOrderDraftApprovedUpdateDto: NoExtra<TicketOrderDraftApprovedUpdateType, T>
    ...
    const registeredAt = ticketOrderDraftApprovedUpdateDto.registeredAt

    return await this.dataSource.transaction('READ UNCOMMITTED', async (manager) => {
      const [ticket] = await this.ticketManager.updateAndReturnEntity(
        manager,
        { id: ticketId, oid },
        {
