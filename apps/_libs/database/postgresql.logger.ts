...
import { NestLogger } from '../common/nest-core/nest-logger'
// import * as util from 'util'

export class PostgresqlLogger implements Logger {
  private readonly logger = new NestLogger(PostgresqlLogger.name)
  ...
  logQueryError(
    ...
    const sqlError = format(query, { language: 'postgresql' })

    this.logger.error('==== 📦 PostgreSQL: ERROR ====')
    this.logger.error(sqlError)
  ...
  logQuerySlow(time: number, query: string, parameters?: any[], _queryRunner?: QueryRunner) {
    console.warn(`==== 📦 PostgreSQL: SLOW QUERY (${time}ms) ====`)
