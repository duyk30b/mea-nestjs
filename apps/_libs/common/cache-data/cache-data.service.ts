...
export class CacheDataService {
  ...
  async getRoleList(oid: number, ridList: number[]) {
    const result: Role[] = []
    for (let i = 0; i < ridList.length; i++) {
      const role = await this.getRole(oid, ridList[i])
      result.push(role)
    }
    return result
  }
  ...
  async getPermissionIdsByUserId(oid: number, uid: number) {
    ...
    if (!this.orgCache[oid].userRoleList) {
      this.orgCache[oid].userRoleList = await this.userRoleRepository.findManyBy({})
    }
    const roleIdList = this.orgCache[oid].userRoleList
      .filter((i) => i.userId === uid)
      .map((i) => i.roleId)

    const roleList = await this.getRoleList(oid, roleIdList)

    let permissionIds = []
    for (let i = 0; i < roleList.length; i++) {
      permissionIds = permissionIds.concat(JSON.parse(roleList[i].permissionIds || '[]'))
    }
    return uniqueArray(permissionIds)
  }
